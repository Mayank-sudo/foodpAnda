import React, { useEffect, useState } from 'react'
import Table from 'react-bootstrap/Navbar'
import './style.css'
import { useParams } from 'react-router-dom'
import  { useSelector} from 'react-redux'

const CardDetails = () => {

     const {id} = useParams();
    //  console.log(id)

    const [data,setData] = useState()
    console.log(data)


    const getdata = useSelector((state)=> state.cartreducer.carts)
//   console.log(getdata)


  const compare = ()=>{
    let compareData = getdata.filter((e)=>{
        return e.id === id
    })
    console.log(compareData)
  }

  useEffect(()=>{
    compare()
  },[id])






  return (
    <div>
        <div className="container mt-3">
            <h2 className='text-center'>Item Details Page</h2>
        </div>
        <section className='container mt-3'>
               {
                data.map((ele)=>{
                    
                })
               }
            <div className="iteamsdetails">
            <div className="items_img" alt="" >
                <img src="https://b.zmtcdn.com/data/pictures/9/18857339/8f53919f1175c08cf0f0371b73704f9b_o2_featured_v2.jpg?output-format=webp"   alt=''/>
            </div>

           <div className='details'>
           <Table>
            <tr>
                <td>
                    <p><strong>Restraunt</strong> : Massala Theory</p>
                    <p><strong>Price</strong> : ₹ 300</p>
                    <p><strong>Dishes</strong> : North Indian, Briyani, Mughal</p>
                    <p><strong>Total</strong>  : ₹  300</p>
                </td>
                <td>
                    <p><strong>Rating :</strong><span style={{background:"green", color: '#fff', padding: '2px 5px', borderRadius: "5px"}}>3.5 ★</span></p>
                    <p><strong>Order Review :</strong><span>1175 order placed in  a day</span></p>
                    <p><strong>Remove   :  </strong><span><i className='fas fa-trash' style={{color: 'red',fontSize:20,cursor: 'pointer'}}></i></span></p>

                </td>
            </tr>
           </Table>
               
           </div>   
            </div>
        </section>
    </div>
  )
}

export default CardDetails